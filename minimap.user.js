...
// @homepage     https://github.com/Tobiasheitor/BRAP-Minimap
// @updateURL    https://raw.githubusercontent.com/Tobiasheitor/BRAP-Minimap/master/minimap.user.js
// @downloadURL  https://raw.githubusercontent.com/Tobiasheitor/BRAP-Minimap/master/minimap.user.js
// @grant        none
// ==/UserScript==
...
//https://raw.githubusercontent.com/Tobiasheitor/BRAP-Minimap/master/README.md

window.baseTepmlateUrl =
  "https://raw.githubusercontent.com/Tobiasheitor/BRAP-Minimap/master";
...
window.addEventListener(
  ...
  function () {
    ...
    document.getElementById("zoom-plus").addEventListener(
      "mousedown",
      function (e) {
        e.preventDefault();
        zooming_in = true;
    ...
    document.getElementById("zoom-minus").addEventListener(
      "mousedown",
      function (e) {
        e.preventDefault();
        zooming_out = true;
...
function exportMd() {
  ...
  Object.keys(template_list).map(function (index, ele) {
    ...
    mdstr += "\n#### " + index;
    mdstr +=
      "\n[![](https://raw.githubusercontent.com/Tobiasheitor/BRAP-Minimap/master/images/" +
      eles.name +
      ")]";
    mdstr +=
      "(http://pixelcanvas.io/@" +
      Math.floor(eles.x + eles.width / 2) +
      "," +
      Math.floor(eles.y + eles.height / 2) +
      ")";
    mdstr += "\n";
...
function loadTemplates() {
  ...
  for (i = 0; i < keys.length; i++) {
    ...
    var temp_xr =
      template_list[template]["x"] * 1 + template_list[template]["width"] * 1;
    var temp_yb =
      template_list[template]["y"] * 1 + template_list[template]["height"] * 1;
    if (
      temp_xr <= x_left ||
      temp_yb <= y_top ||
      temp_x >= x_right ||
  ...
  if (needed_templates.length == 0) {
    if (zooming_in == false && zooming_out == false) {
      document.getElementById("minimap-box").style.display = "none";
      document.getElementById("minimap-text").style.display = "block";
      document.getElementById("minimap-text").innerHTML =
        "Não há templates nesse local";
    }
  } else {
    document.getElementById("minimap-box").style.display = "block";
    document.getElementById("minimap-text").style.display = "none";
...
function loadImage(imagename) {
  ...
  if (cachebreaker != null)
    image_list[imagename].src =
      window.baseTepmlateUrl + "/images/" + template_list[imagename].name;
  else
    image_list[imagename].src =
      window.baseTepmlateUrl + "/images/" + template_list[imagename].name;
...
function drawCursor() {
  ...
  var y_top = y_window * 1 - minimap.height / zoomlevel / 2;
  var y_bottom = y_window * 1 + minimap.height / zoomlevel / 2;
  ctx_minimap_cursor.clearRect(
    0,
    0,
    minimap_cursor.width,
    minimap_cursor.height
  );
